...
System.register(['jquery.flot', 'jquery.flot.selection', 'jquery.flot.time', 'jquery.flot.stack', 'jquery.flot.stackpercent', 'jquery.flot.fillbelow', 'jquery.flot.crosshair', 'jquery.flot.dashes', './jquery_flot_events', 'jquery', 'lodash', 'moment', 'app/core/utils/kbn', 'app/core/utils/ticks', 'app/core/core', './graph_tooltip', './threshold_manager', 'app/features/annotations/all', './histogram'], function (_export, _context) {
  "use strict";

  var $, _, moment, kbn, tickStep, appEvents, coreModule, GraphTooltip, ThresholdManager, EventManager, convertValuesToHistogram, getSeriesValues;

  return {
    setters: [function (_jqueryFlot) {}, function (_jqueryFlotSelection) {}, function (_jqueryFlotTime) {}, function (_jqueryFlotStack) {}, function (_jqueryFlotStackpercent) {}, function (_jqueryFlotFillbelow) {}, function (_jqueryFlotCrosshair) {}, function (_jqueryFlotDashes) {}, function (_jquery_flot_events) {}, function (_jquery) {
      $ = _jquery.default;
    }, function (_lodash) {
      _ = _lodash.default;
    }, function (_moment) {
    ...
    }, function (_graph_tooltip) {
      GraphTooltip = _graph_tooltip.default;
    }, function (_threshold_manager) {
      ThresholdManager = _threshold_manager.ThresholdManager;
    }, function (_appFeaturesAnnotationsAll) {
      EventManager = _appFeaturesAnnotationsAll.EventManager;
    }, function (_histogram) {
      convertValuesToHistogram = _histogram.convertValuesToHistogram;
      getSeriesValues = _histogram.getSeriesValues;
    }],
    execute: function () {
          ...
          link: function link(scope, elem) {
            ...
            function drawHook(plot) {
              ...
              if (panel.yaxes[0].label && panel.yaxes[0].show) {
                var yaxisLabel = $("<div class='axisLabel left-yaxis-label flot-temp-elem'></div>").text(panel.yaxes[0].label).appendTo(elem);
              }

              // add right axis labels
              if (panel.yaxes[1].label && panel.yaxes[1].show) {
                var rightLabel = $("<div class='axisLabel right-yaxis-label flot-temp-elem'></div>").text(panel.yaxes[1].label).appendTo(elem);
              }
                ...
                case 'histogram':
                  {
                    ...
                    if (data.length && values.length) {
                      var histMin = _.min(_.map(data, function (s) {
                        return s.stats.min;
                      }));
                      var histMax = _.max(_.map(data, function (s) {
                        return s.stats.max;
                      }));
                      var ticks = panel.xaxis.buckets || panelWidth / 50;
                      bucketSize = tickStep(histMin, histMax, ticks);
                      var histogram = convertValuesToHistogram(values, bucketSize);
            ...
            function addXHistogramAxis(options, bucketSize) {
              ...
              if (data.length && bucketSize) {
                ticks = _.map(data[0].data, function (point) {
                  return point[0];
                });
                min = _.min(ticks);
                max = _.max(ticks);
